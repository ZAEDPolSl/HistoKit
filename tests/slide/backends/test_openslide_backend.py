...
from src.histo_kit.slide.backends.openslide import OpenSlideBackend
...
def backend(tmp_path: Path, request):
    ...
    return OpenSlideBackend(str(path))

def test_backend_initializes(backend):
    assert backend.level_count > 0
...
def test_level_downsamples(backend):
    assert len(backend.level_downsamples) == len(backend.level_dimensions)
