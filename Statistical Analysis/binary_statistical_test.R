...
df <- read.csv(
  "Data/HistoKit (no postprocessing)/binary_metrics.csv",
  check.names = FALSE
)
...
df <- df %>%
  rename_with(
    ~ unname(metrics_names[.x]),
    .cols = any_of(names(metrics_names))
  )

metrics <- c("Accuracy", "Dice", "FDR", "Sensitivity", 
             "Jaccard", "Precision", "NPV", "F1", "Specificity")
...
for (metric in names(combined_heatmaps)) {
  ...
  output_path <- file.path(
    "Data/HistoKit (no postprocessing)/binary/heatmaps_pvalue_effect_size/",
    paste0(
      tolower(metric),
