...
from histo_kit.grand_qc.statistics import calculate_stats, plot_stats
...
# Common settings
parser.add_argument('--wsi_dir', type=str, help='Input directory with WSIs', default="/mnt/mmanas/mmarczyk/!TMP/Modena_Mice/")
...
parser.add_argument('--out_dir', type=str, help='Output directory', default='/mnt/warehouse/Projects/HE/Modena_Mice_10x_changed_colors/')
...
parser.add_argument('--device', help='Device used for artifacts detection: cuda or cpu', choices=["cuda", "cpu"],default="cuda:0")
...
parser.add_argument('--grandqc_model', help='Path to GrandQC model weights (model for 10x magnification is used by default).',default="/mnt/warehouse/jmerta/HistoKit/models/GrandQC_MPP1.pth", type=str)
...
if __name__ == "__main__":
    ...
    if args.wsi_dir is not None:
        all_slides = glob.glob(os.path.join(args.wsi_dir, '*.ndpi'))
    ...
    if args.calculate_stats:
        ...
        out_csv_region = os.path.join(args.out_dir, 'artifacts_stats_region.csv')
        if os.path.exists(out_csv_region):
            os.remove(out_csv_region)

        out_csv = os.path.join(args.out_dir, 'artifacts_stats_wsi.csv')
        ...
        for s_f in tqdm(tis_art, total=len(tis_art), desc="Calculating artifacts statistics..."):
            basename = get_basename(s_f)
            try:
                df_region, df_wsi = calculate_stats(s_f)

                write_header = not os.path.exists(out_csv_region)
                df_region.to_csv(out_csv_region, mode='a', index=False, header=write_header)

                write_header = not os.path.exists(out_csv)
                df_wsi.to_csv(out_csv, mode='a', index=False, header=write_header)
        ...
        try:
            df_region = pd.read_csv(out_csv_region)
            plot_stats(df_region, out_csv_region.replace(".csv", ".png"), title= "Distribution of Artifact Percentages per Region")

            df_wsi = pd.read_csv(out_csv)
            plot_stats(df_wsi, out_csv.replace(".csv", ".png"), title = "Distribution of Artifact Percentages per WSI")
        except Exception as e:
            print(f"There was an error during plotting statistics - {str(e)}")
