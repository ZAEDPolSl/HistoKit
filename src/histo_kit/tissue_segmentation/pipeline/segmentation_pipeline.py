...
from ..savers.base_server import BaseSaver
from ...slide.slide import Slide
from .config import PipelineConfig
from .registry import SEGMENTERS, SAVERS
...
class TissueSegPipeline:
    ...
    def run(self, slide_path):
        slide = Slide(slide_path)

        result = self.segmenter.segment(slide)  # dict
        save_info = self.saver.save(slide_path, result)

        if self.visualizations is not None:
            self.visualizations.run(
                data=result,
                slide=slide,
                out_dir=Path(self.segmenter.config.out_dir)
            )
