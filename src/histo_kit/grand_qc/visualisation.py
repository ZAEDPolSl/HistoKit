...
def make_overlay(slide, wsi_heatmap_im, bg_mask, vis_size):
    ...
    overlay = cv2.addWeighted(np.array(slide_reduced), 0.4, np.array(heatmap_temp), 0.6, 0)
    mask = np.all(overlay == [255, 255, 255], axis=-1)
    ...
    overlay[mask] = np.array(slide_reduced)[mask]
    overlay = cv2.drawContours(overlay, contours, -1, (0, 0, 255), 1)
    return overlay
