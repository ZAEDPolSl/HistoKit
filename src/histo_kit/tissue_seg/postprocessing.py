...
def remove_black_pen(img, thr_low, thr_back, radius):
    ...
    the image in the HSV color space and refining the result using morphological
    ...
    thr_low : float
        Lower threshold applied to the Value (V) channel in HSV space to
    ...
    >>> mask = remove_black_pen(image, thr_low=0.2, thr_back=thr_back, radius=3)
...
def remove_small_objects(mask):
    ...
    Objects smaller than a calculated threshold (roughly 5% of image area)
    are removed.
