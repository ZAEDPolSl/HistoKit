...
parser.add_argument('--wsi_dir', type=str, help='Input directory with WSIs', default='/mnt/data/Tmp/')
...
for slide_file in tqdm(slides):
    try:
        ...
        p_s, patch_n_w_l0, patch_n_h_l0, w_l0, h_l0, obj_power = slide_info(slide, args.patch_size_model, args.grandqc_mpp, mpp_slide)

        h, w = res_dict["mask"].shape
        tis_det = Image.fromarray(1 - res_dict["mask"].astype(np.uint8))
        tis_det = np.array(tis_det.resize((int(w * MAG_MODEL/args.tissdet_mag), int(h * MAG_MODEL/args.tissdet_mag)), Image.Resampling.NEAREST))

        map_tis, full_mask, tis_det = slide_process_single(model_grandQC, tis_det, slide, patch_n_w_l0, patch_n_h_l0, p_s,
                                                   args.patch_size_model, args.encoder_model,args.encoder_model_weights,
                                                   args.device, args.grandqc_mpp, mpp_slide, w_l0, h_l0, vis_size)

        # save color grandQC artifacts map
        map_tis = save_rescaled(map_tis, vis_size, os.path.join(GRANDQC_MAP_VIS, f'{basename}.png'), rescale_method=Image.Resampling.NEAREST)
